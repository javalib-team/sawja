...
type instr =
  ...
  | InvokeStatic of
      var option * JBasics.class_name * method_signature * expr list
  | InvokeVirtual of
      var option * expr * virtual_call_kind * method_signature * expr list
  | InvokeNonVirtual of
      var option * expr * JBasics.class_name * method_signature * expr list
  | InvokeDynamic of
      var option
      * JBasics.bootstrap_method
      ...
      ( match u with
        ...
        | Conv c -> (
            ...
            | L2I | F2I | D2I | I2B | I2C | I2S ->
              `Int )
        | ArrayLength | InstanceOf _ ->
          `Int
        | _ ->
          assert false )
  | Binop (ArrayLoad t, _, _) ->
    t
  | Binop (b, _, _) ->
    TBasic
      ( match b with
        | ArrayLoad _ ->
          assert false
        | Add t | Sub t | Mult t | Div t | Rem t -> (
            match t with
            | `Int2Bool ->
              `Int
            | `Long ->
...
let instr_jump_to = function Ifd (_, n) | Goto n -> Some n | _ -> None
...
let rec print_expr' ?(show_type = true) first_level = function
  ...
  | Field (e, c, f) ->
    Printf.sprintf "%s.%s"
      (print_expr' ~show_type false e)
      (JUtil.print_field c f)
  | StaticField (c, f) ->
    Printf.sprintf "%s.%s" (JPrint.class_name c) (fs_name f)
  | Const i ->
    print_const i
  | Unop (ArrayLength, e) ->
    Printf.sprintf "%s.length" (print_expr' ~show_type false e)
  | Unop (Cast t, e) ->
    Printf.sprintf "(%s) %s" (print_typ (TObject t))
      (print_expr' ~show_type true e)
  | Unop (op, e) ->
    Printf.sprintf "%s(%s)" (print_unop op) (print_expr' ~show_type true e)
  | Binop (ArrayLoad t, e1, e2) ->
    if show_type then
      Printf.sprintf "%s[%s]:%s"
        (print_expr' ~show_type false e1)
        (print_expr' ~show_type true e2)
        (print_typ t)
    else
      Printf.sprintf "%s[%s]"
        (print_expr' ~show_type false e1)
  ...
  | Binop (Div _, e1, e2) ->
    JUtil.bracket first_level
      (Printf.sprintf "%s/%s"
         ...
         (print_expr' ~show_type false e2))
  | Binop (op, e1, e2) ->
    Printf.sprintf "%s(%s,%s)" (print_binop op)
      (print_expr' ~show_type true e1)
      (print_expr' ~show_type true e2)
...
let print_stackmap ?(show_type = true) = function
  ...
  | x :: q ->
    List.fold_left
      (fun s t -> Printf.sprintf "%s :: %s" (print_oexpr ~show_type t) s)
      (print_oexpr ~show_type x) q

let print_instr ?(show_type = true) = function
  | Nop ->
    "nop"
  | AffectVar (x, e) ->
    Printf.sprintf "%s := %s" (var_name_g x) (print_expr' ~show_type true e)
  | AffectStaticField (c, f, e) ->
    Printf.sprintf "%s.%s := %s" (JPrint.class_name c) (fs_name f)
      (print_expr' ~show_type true e)
  | AffectField (e1, c, f, e2) ->
    Printf.sprintf "%s.%s := %s"
      (print_expr' ~show_type false e1)
      (JUtil.print_field c f)
      (print_expr' ~show_type true e2)
  | AffectArray (e1, e2, e3) ->
    Printf.sprintf "%s[%s] := %s"
      (print_expr' ~show_type false e1)
      (print_expr' ~show_type true e2)
      (print_expr' ~show_type true e3)
  | Goto i ->
    Printf.sprintf "goto %d" i
  | Ifd (g, el) ->
    Printf.sprintf "if (%s) goto %d" (print_cmp g) el
  | Throw e ->
    Printf.sprintf "throw %s" (print_expr' ~show_type false e)
  | Return None ->
  ...
  | NewArray (x, c, le) ->
    Printf.sprintf "%s := new %s%s" (var_name_g x) (JPrint.value_type c)
      (JUtil.print_list_sep ""
         (fun e -> Printf.sprintf "[%s]" (print_expr' ~show_type true e))
         le)
  | InvokeStatic (None, c, ms, le) ->
    Printf.sprintf "%s.%s(%s)" (JPrint.class_name c) (ms_name ms)
      (JUtil.print_list_sep "," (print_expr' ~show_type true) le)
  | InvokeStatic (Some x, c, ms, le) ->
  ...
  | InvokeDynamic (Some x, bm, ms, le) ->
    ...
    Printf.sprintf "%s := CLOSURE[%s[%d],%s](%s)" (var_name_g x)
      (print_method_handle bm.bm_ref)
      (List.length bm.bm_args) (ms_name ms)
      (JUtil.print_list_sep "," (print_expr' ~show_type true) le)
    ...
    Printf.sprintf "%s%s.%s.%s(%s)"
      ( match r with
        ...
        | Some x ->
          Printf.sprintf "%s := " (var_name_g x) )
      (print_expr' ~show_type false e1)
      (JPrint.class_name kd) (ms_name ms)
      (JUtil.print_list_sep "," (print_expr' ~show_type true) le)
  | MonitorEnter e ->
    Printf.sprintf "monitorenter(%s)" (print_expr' ~show_type true e)
  | MonitorExit e ->
    Printf.sprintf "monitorexit(%s)" (print_expr' ~show_type true e)
  | MayInit c ->
    Printf.sprintf "mayinit %s" (JPrint.class_name c)
  | Check c -> (
      match c with
      | CheckNullPointer e ->
        Printf.sprintf "notnull %s" (print_expr' ~show_type true e)
      | CheckArrayBound (a, i) ->
        Printf.sprintf "checkbound %s[%s]"
          (print_expr' ~show_type true a)
          (print_expr' ~show_type true i)
      | CheckArrayStore (a, v) ->
        Printf.sprintf "checkstore %s[] <- %s"
          (print_expr' ~show_type true a)
          (print_expr' ~show_type true v)
      | CheckNegativeArraySize e ->
        Printf.sprintf "checknegsize %s" (print_expr' ~show_type true e)
      | CheckCast (e, t) ->
        Printf.sprintf "checkcast %s:%s"
          (print_expr' ~show_type true e)
...
type bir =
  { bir_vars: var array
  (** All variables that appear in the method. [vars.(i)] is the variable of index [i]. Caution, when we are in bir SSA, it contains both the bir variables (with lower index) and birSSA variables. *)
  ; bir_dico: dictionary
  ...
  if i < 0 then acc
    ...
    let new_acc =
      app_phi_nodes phi_simpl i m.bir_preds.(i) m.bir_phi_nodes.(i)
        (Printf.sprintf "%3d: %s" i (print_instr m.bir_code.(i)) :: acc)
    in
...
let bir_exception_edges m = generic_exception_edges m.bir_code m.bir_exc_tbl
...
let topE = function
  | [] ->
    raise Bad_stack
  | Uninit _ :: _ ->
    raise Uninit_is_not_expr
  | E e :: _ ->
    e

let getE = function Uninit _ -> raise Uninit_is_not_expr | E e -> e
...
let succs code i = normal_next code i @ compute_handlers code i
...
let mapi f g =
  let rec aux i = function [] -> [] | x :: q -> f i x :: aux (g i x) q in
  aux 0
...
module BCV = struct
  ...
  let conv_array_type t =
    match t with
    | `Int | `Short | `Char | `Int2Bool | `ByteBool ->
      Int
    | `Float ->
      Float
    | `Object ->
  ...
  let size = function
    | Top ->
      assert false
    | Int | Float | Object | Null | Array _ | Top32 ->
      Op32
    | Double | Long | Top64 ->
      Op64

  let is32_basic = function
    | `Int | `Short | `Char | `Byte | `Bool | `Float ->
      true
    | `Long | `Double ->
      false
  ...
  let rec leq_value_type v1 v2 =
    match (v1, v2) with
    | TObject v1, TObject v2 ->
      leq_object_type v1 v2
    | TBasic t1, TBasic t2 ->
      t1 = t2
    | _, _ ->
      false
  ...
  and leq_object_type v1 v2 =
    match (v1, v2) with
    | TClass c1, TClass c2 ->
      cn_equal c1 c2 || cn_equal c2 java_lang_object
    | TArray v1, TArray v2 ->
      leq_value_type v1 v2
    | TArray _, TClass c ->
      cn_equal c java_lang_object
    | _, _ ->
      false

  let ( <= ) x y =
    x == y || x = y || y = Top || x = Null
    ||
    match y with
    | Top64 ->
      x = Double || x = Long || x = Top64
    | Object -> (
        match x with Array _ -> true | _ -> false )
    | Top32 -> (
        match x with
        | Array _ | Object | Int | Float | Top32 ->
          true
        | _ ->
          false )
    | Array y -> (
        match x with Array x -> leq_value_type x y | _ -> false )
    | _ ->
      false
  ...
  let rec lub_value_type v1 v2 =
    match (v1, v2) with
    | TObject v1, TObject v2 ->
      Some (TObject (lub_object_type v1 v2))
    | _, _ ->
      None
  ...
  let lub x y =
    if x <= y then y
      ...
      match x with
      | Int | Float | Object -> (
          match y with Top64 | Double | Long -> Top | _ -> Top32 )
      | Double | Long -> (
          match y with Double | Long -> Top64 | _ -> Top )
      | Array x -> (
          match y with
          | Array y -> (
              match lub_value_type x y with None -> Object | Some v -> Array v )
          | Int | Float ->
            Top32
          | _ ->
            Top )
    ...
    | JCode.OpConst x ->
      fun (s, l) ->
        let c =
          ...
          | `ANull ->
            Null
          | `String _ | `MethodHandle _ | `MethodType _ | `Class _ ->
            Object
          | `Byte _ | `Short _ | `Int _ ->
            Int
          | `Long _ ->
  ...
  let run verbose cm code =
    ...
    ( try loop ws with
      | GetNotFound ->
        Printf.printf "GET_NOT_FOUND !\n" ;
        assert false
      | BadLoadType ->
        Printf.printf "BAD_LOAD_TYPE !\n"
      | ArrayContent ->
        assert false ) ;
  ...
  let run bcv ?(verbose = false) cm code =
    if bcv then run verbose cm code else run_dummy code
end
...
let remove_dim t n =
  ...
  let rec aux t n =
    match t with
    | TClass _ ->
      raise Bad_Multiarray_dimension (* bad dimension *)
    | TArray t -> (
        if n = 1 then t
        else
          match t with
...
let is_arrayload = function ArrayLoad _ -> true | _ -> false
...
let is_in_expr test_var test_static test_field test_array =
  let rec aux expr =
    ...
    | StaticField (c, f) ->
      test_static c f
    | Field (e, c, f) ->
      test_field c f || aux e
    | Var (_, x) ->
      test_var x
    | Unop (_, e) ->
      aux e
    | Binop (s, e1, e2) ->
      (test_array && is_arrayload s) || aux e1 || aux e2
  in
  aux
...
let var_in_expr test_var =
  let rec aux expr =
    ...
    | Field (e, _, _) ->
      aux e
    | Var (t, x) ->
      if test_var x then Some t else None
    | Unop (_, e) ->
      aux e
    | Binop (_, e1, e2) -> (
        match aux e1 with None -> aux e2 | Some t -> Some t )
  in
  aux
...
let replace_in_expr test_var test_static var =
  let rec aux expr =
    ...
    | Const _ ->
      expr
    | StaticField (c, f) ->
      if test_static c f then Var (fs_type f, var) else expr
    | Field (e, c, f) ->
      Field (aux e, c, f)
    | Var (t, x) ->
      if test_var x then Var (t, var) else expr
    | Unop (s, e) ->
      Unop (s, aux e)
    | Binop (s, e1, e2) ->
...
let is_in_stack test_var test_static =
  let rec aux stack =
    ...
    | [] ->
      false
    | E e :: _ when is_in_expr test_var test_static (fun _ _ -> false) false e
      ->
      true
    | _ :: s ->
      aux s
...
let var_in_stack test_var =
  let rec aux stack =
    ...
    | [] ->
      None
    | E e :: s -> (
        match var_in_expr test_var e with Some t -> Some t | None -> aux s )
    | _ :: s ->
      aux s
  in
  aux
...
let replace_in_stack test_var test_static var =
  let rec aux stack =
    ...
    | e' :: s -> (
        match e' with
        | E e ->
          E (replace_in_expr test_var test_static var e) :: aux s
        | Uninit _ ->
          e' :: aux s )
  in
  aux

let is_var_in_stack x stack = var_in_stack (( = ) x) stack
...
let is_var_in_expr_not_var x e =
  (* warning we use ocaml equality *)
...
let replace_var_in_expr x y = replace_in_expr (( = ) x) (fun _ _ -> false) y
...
let temp_in_expr acc expr =
  let rec aux acc expr =
    match expr with
    | Const _ | StaticField _ ->
      acc
    | Field (e, _, _) ->
      aux acc e
    | Var (_, (_, x)) -> (
        match x with TempVar i -> Ptset.add i acc | _ -> acc )
    | Unop (_, e) ->
      aux acc e
    | Binop (_, e1, e2) ->
      aux (aux acc e1) e2
...
let temp_in_opexpr acc = function Uninit _ -> acc | E e -> temp_in_expr acc e

(* Return the set of Tempvar found in the stack. *)
let temp_in_stack s = List.fold_left temp_in_opexpr Ptset.empty s
...
let fresh_in_stack ssa fresh_counter s =
  if ssa then (
    ...
    if Ptset.is_empty set then 0 else Ptset.max_elt set + 1
...
let to_addr3_binop dico mode binop ssa fresh_counter s =
  ...
  | Addr3 -> (
      ...
      match binop with
      | (Div jvm_t | Rem jvm_t) when jvm_t = `Int2Bool || jvm_t = `Long ->
        let q = topE s in
        let e = Binop (binop, topE (pop s), topE s) in
        ( E (Var (type_of_expr e, x)) :: pop2 s
        , [Check (CheckArithmetic q); AffectVar (x, e)] )
      | _ ->
        let e = Binop (binop, topE (pop s), topE s) in
  ...
  | _ -> (
      ...
      | (Div jvm_t | Rem jvm_t) when jvm_t = `Int2Bool || jvm_t = `Long ->
        ...
        , [Check (CheckArithmetic q)] )
      | _ ->
        (E (Binop (binop, topE (pop s), topE s)) :: pop2 s, []) )

let to_addr3_const dico mode ssa fresh_counter c s instrs next_store
    next_is_junc_point_or_a_goto =
  match mode with
  | Addr3 -> (
...
let to_addr3_unop dico mode unop ssa fresh_counter s instrs =
  ...
  | _ ->
    (E (Unop (unop, topE s)) :: pop s, instrs)

let make_tempvar dico ssa fresh_counter s next_store =
  match next_store with
  | None ->
    make_var dico (TempVar (fresh_in_stack ssa fresh_counter s))
  | Some x -> (
...
and type_of_array_content t e =
  ...
  | TObject (TArray t) ->
    t (* can this happen ? *)
  | _ -> (
      (* we use the type found in the OpArrayLoad argument *)
      match t with
      | `Int | `Short | `Char | `ByteBool ->
        TBasic `Int
      | `Long ->
        TBasic `Long
      | `Float ->
...
let bc2bir_instr dico mode pp_var ch_link ssa folding fresh_counter i load_type
  ...
  | JCode.OpArrayLoad t -> (
      ...
      | Addr3 ->
        ...
        , [ Check (CheckNullPointer a)
          ...
          ; AffectVar (x, Binop (ArrayLoad t, a, idx)) ] )
      | _ ->
  ...
  | JCode.OpIInc (a, b) -> (
      ...
      | Some t -> (
          ...
          | Addr3 ->
            let y =
              make_tempvar dico ssa fresh_counter
                (E (Var (TBasic `Int, x)) :: s)
                None
            in
  ...
  | JCode.OpPutField (c, f) as instr ->
    ...
    let instrs =
      if ch_link then
        ...
        ; Check (CheckNullPointer r)
        ; AffectField (r, c, f, topE s) ]
  ...
  | JCode.OpCmp op -> (
      ...
      | `DL ->
        to_addr3_binop dico mode (CMP DL) ssa fresh_counter s
      | `FG ->
        to_addr3_binop dico mode (CMP FG) ssa fresh_counter s
      | `FL ->
  ...
  | JCode.OpIf (x, n) ->
    let guard =
      match x with
      | `NonNull ->
        (`Ne, topE s, Const `ANull)
      | `Null ->
        (`Eq, topE s, Const `ANull)
      | (`Eq | `Ge | `Gt | `Le | `Lt | `Ne) as c -> (
          match topE s with
          | Binop (CMP _, e1, e2) ->
            (c, e1, e2)
          | e ->
            (c, e, Const (`Int (Int32.of_int 0))) )
    in
    let target = i + n in
  ...
  | JCode.OpTableSwitch (def, min, _, tbl) ->
    ...
    , Array.fold_right
        (fun (guard, i) l ->
           to_addr3_ifd dico mode ssa fresh_counter s guard i @ l )
        (Array.mapi
           (fun idx j ->
              ( (`Eq, topE s, Const (`Int (Int32.add (Int32.of_int idx) min)))
              , j + i ) )
           tbl)
        [Goto (def + i)] )
  | JCode.OpLookupSwitch (def, pairs) ->
    ...
    , List.fold_right
        (fun (c, j) l ->
           to_addr3_ifd dico mode ssa fresh_counter s
             (`Eq, topE s, Const (`Int c))
             (j + i)
           @ l )
        pairs
        [Goto (def + i)] )
  ...
  | JCode.OpGetStatic (c, f) as instr -> (
      ...
      | Addr3 ->
	   ...
	   have been generated*)
        let x = make_tempvar dico ssa fresh_counter s next_store in
        ( E (Var (fs_type f, x)) :: s
        , instrs @ [AffectVar (x, StaticField (c, f))] )
      | _ ->
        let instrs =
          if ch_link then [Check (CheckLink instr); MayInit c] else [MayInit c]
        in
  ...
  | JCode.OpInvoke (x, ms) as instr -> (
      ...
      | `Static (_, c) -> (
          ...
          | None ->
            let instrs =
              if ch_link then [Check (CheckLink instr); MayInit c]
              else [MayInit c]
            in
            let s, instrs =
              clean dico ssa fresh_counter is_heap_sensible_element_in_expr s
	       ...
	       temp variable*)
            ...
            ( popn (List.length (ms_args ms)) s
            , instrs @ [InvokeStatic (None, c, ms, params)] )
          | Some t ->
            let instrs =
              if ch_link then [Check (CheckLink instr); MayInit c]
              else [MayInit c]
      ...
      | `Dynamic bm -> (
          match ms_rtype ms with
          | None ->
            let instrs = if ch_link then [Check (CheckLink instr)] else [] in
            let s, instrs =
              clean dico ssa fresh_counter is_heap_sensible_element_in_expr s
                instrs
            in
	       ...
	       temp variable*)
            let params = param (List.length (ms_args ms)) s in
            ( popn (List.length (ms_args ms)) s
            , instrs @ [InvokeDynamic (None, bm, ms, params)] )
          | Some t ->
            let instrs = if ch_link then [Check (CheckLink instr)] else [] in
            let s, instrs =
              clean dico ssa fresh_counter is_heap_sensible_element_in_expr s
      ...
      | x -> (
          ...
          | Uninit (c, j) ->
            ...
            let instrs =
              ...
              in
              if ch_link then Check (CheckLink instr) :: instrs else instrs
            in
          ...
          | E e0 -> (
              ...
              let ins target =
                ...
                | `Virtual o ->
                  [ InvokeVirtual
                      (target, this, VirtualCall o, ms, param nb_args s) ]
                | `Interface c ->
                  [ InvokeVirtual
                      (target, this, InterfaceCall c, ms, param nb_args s) ]
                | `Special (_, c) ->
              ...
              let checks =
                ...
                else [Check (CheckNullPointer e0)]
              in
              match ms_rtype ms with
              | None ->
                clean dico ssa fresh_counter is_heap_sensible_element_in_expr
  ...
  | JCode.OpNewArray t as instr ->
    ...
    if ch_link then
      ...
      | TObject _ ->
        ( E (Var (TObject (TArray t), x)) :: pop s
        , [ Check (CheckLink instr)
          ; Check (CheckNegativeArraySize dim)
          ; NewArray (x, t, [dim]) ] )
    else
      ( E (Var (TObject (TArray t), x)) :: pop s
      , [Check (CheckNegativeArraySize dim); NewArray (x, t, [dim])] )
  | JCode.OpArrayLength -> (
      ...
      | Addr3 ->
        let x = make_tempvar dico ssa fresh_counter s next_store in
        ( E (Var (TBasic `Int, x)) :: pop s
        , [Check (CheckNullPointer a); AffectVar (x, Unop (ArrayLength, a))]
        )
      | _ ->
...
let is_branchvar_in_stack succs =
  let test = function _, BranchVar (i, _) -> List.mem i succs | _ -> false in
          ...
          | E e ->
            ...
            let l =
              List.map
                (fun j ->
                   AffectVar
                     (make_var dico (BranchVar (j, i)), Var (type_of_expr e, x))
                )
                succs
            in
    ...
    let rec aux i = function
      ...
      | e :: q -> (
          ...
          | E e ->
            let l =
              List.map
                (fun j -> AffectVar (make_var dico (BranchVar (j, i)), e))
                succs
            in
            l @ aux (i + 1) q )
    in
  ...
  let rec aux i = function
    ...
    | e :: q -> (
        ...
        | Uninit _ ->
          e :: aux (i + 1) q
        | E e ->
          E (Var (type_of_expr e, make_var dico (BranchVar (pc', i))))
          :: aux (i + 1) q )
  in
  aux 0 stack
...
let value_compare e1 e2 =
  match (e1, e2) with Var (_, x), Var (_, y) -> x = y | _ -> e1 = e2
...
let value_compare_stack s1 s2 = List.for_all2 value_compare s1 s2
...
module FastCheckInfoDebug = struct
  ...
  *)
  ...
  let pp_var2vardeb f x i =
    match f x i with None -> NoName | Some s -> Name s
	 ...
	 local_variable_table information*)
      ...
      let check_all as_succ dom_bj darray jump_pc =
	   ...
	   local_variable_table information *)
        ...
        Array.iteri
          (fun i _ ->
             match (Ptmap.mem i dom_bj, Ptmap.mem i darray.(jump_pc)) with
		...
		*)
             | true, true ->
               ...
               if not (bj = aj) then
                 ...
                 else if aj = UnDef then ()
                 else raise (InconsistentDebugInfo (pc, jump_pc, i))
		...
		*)
             | true, false ->
               if jump_pc > pc then
                 if Ptmap.find i dom_bj = NoName then
        ...
        try
          ( List.fold_left
              (fun as_succ i ->
                 ...
                 if j_targ (* case (3)*) then
                   check_all as_succ cur_context darray i (* case (2) *)
                 else (
...
module CheckInfoDebug = struct
  module Lat = struct
    ...
    let join =
      Ptmap.merge (fun ninfo1 ninfo2 ->
          if not (ninfo1 = ninfo2) then FastCheckInfoDebug.UnDef else ninfo1 )

    let get m x = Ptmap.find x m
    ...
    let to_string ab =
      let dom_to_string v =
        match v with
        | FastCheckInfoDebug.Name s ->
          "Name(" ^ s ^ ")"
        | FastCheckInfoDebug.NoName ->
          "NoName"
        | FastCheckInfoDebug.UnDef ->
          "UnDef"
      in
      Ptmap.fold
        (fun i vdom -> Printf.sprintf "%d:%s %s" i (dom_to_string vdom))
        ab ""
  end

  type pc = int

  type transfer = NoP | KillGen of var * int

  let transfer_to_string = function
    | NoP ->
      "NoP"
    | KillGen (x, i) ->
      Printf.sprintf "KillGen(%s,%d)" (var_name_g x) i
  ...
  let eval_transfer = function
    ...
    | KillGen (x, _i) -> (
        ...
        | Some xnum -> (
            ...
            | None ->
              Ptmap.add xnum FastCheckInfoDebug.NoName
            | Some name ->
  ...
  let gen_instrs i = function
    ...
    | InvokeNonVirtual (Some x, _, _, _, _) ->
      if var_orig x then [(KillGen (x, i), i + 1)] else [(NoP, i + 1)]
    | MonitorEnter _
    | MonitorExit _
    | AffectStaticField _
    | AffectField _
    ...
    List.fold_right
      (fun (_, x) ->
         ...
         | Some i -> (
             match var_name_debug x with
             | None ->
               Ptmap.add i FastCheckInfoDebug.NoName
             | Some s ->
               Ptmap.add i (FastCheckInfoDebug.Name s) )
         | None ->
           assert false )
      params Ptmap.empty
  ...
  let rec vars = function
    ...
    | Field (e, _, _) | Unop (_, e) ->
      vars e
    | Var (_, x) ->
      if var_orig x then VarSet.singleton x else VarSet.empty
  ...
  let check_info_on_vars_access code res =
    let get_accessed_vars =
      let vars_in_check = function
        | CheckNullPointer e
        | CheckNegativeArraySize e
        | CheckCast (e, _)
        | CheckArithmetic e ->
          vars e
        | CheckArrayBound (e1, e2) | CheckArrayStore (e1, e2) ->
          VarSet.union (vars e1) (vars e2)
        | CheckLink _ ->
          VarSet.empty
      in
      ...
      | AffectStaticField (_, _, e) ->
        vars e
      | AffectField (e1, _, _, e2) | Ifd ((_, e1, e2), _) ->
        VarSet.union (vars e1) (vars e2)
      | AffectArray (e1, e2, e3) ->
      ...
      | InvokeStatic (_, _, _, el) ->
        List.fold_left
          ...
          VarSet.empty el
      | InvokeVirtual (_, e, _, _, el) | InvokeNonVirtual (_, e, _, _, el) ->
        List.fold_left (fun set e -> VarSet.union (vars e) set) (vars e) el
      | Return e_opt -> (
      ...
      (fun i ins ->
         ...
         VarSet.iter
           (fun var ->
              try
                let bcvar =
                  match bc_num var with Some i -> i | None -> assert false
                in
                let var_name =
                  ...
                  | Some s ->
                    FastCheckInfoDebug.Name s
                in
                if not (Lat.get resi bcvar = var_name) then
...
let bc2ir dico mode ch_link ssa folding pp_var jump_target load_type
  ...
  let rec loop as_ts_jump ins ts_in as_in pc fresh_counter
        ...
        with Not_found ->
          ...
          if List.exists (fun e -> pc = e.JCode.e_handler) code.JCode.c_exc_tbl then
            ...
            , [ E
                  (Var
                     ( TObject (TClass java_lang_object)
                     , make_var dico (CatchVar pc) )) ] )
          else (* this is a back jump target *)
            ([], [])
      else (ts_in, as_in)
    in
    ...
    if List.length as_out > 0 then
      if List.exists (fun j -> j < pc) succs then
        raise NonemptyStack_backward_jump ;
    let branch_assigns = para_assign dico pc jump_succs as_out in
    let ins =
      if is_jump_instr op then (pc, branch_assigns @ instrs) :: ins
      else (pc, instrs @ branch_assigns) :: ins
    in
    let as_ts_jump =
      List.fold_left
        (fun as_jump pc' ->
  ...
  in
  if Array.length code.JCode.c_code = 0 then []
...
let gen_params dico pp_var cm =
  if cm.cm_static then
    mapi
      ...
      (fun i t ->
         match convert_field_type t with Op32 -> i + 1 | Op64 -> i + 2 )
      (ms_args cm.cm_signature)
  else
    ( TObject (TClass java_lang_object)
    , make_var dico (OriginalVar (0, pp_var 0 0)) )
    :: mapi
...
let compress_ir handlers ir jump_target =
  ...
  let rec aux0 pc0 pcl = function
    | [] ->
      [(pc0, [(pcl, Nop)])]
    | (pc, _) :: _ as l when jump_target.(pc) || Ptset.mem pc h_ends ->
      (pc0, [(pcl, Nop)]) :: aux l
    | (pc1, []) :: q ->
      aux0 pc0 (pc1 :: pcl) q
    | (pc, instrs) :: q ->
      (pc, List.map (fun i -> (pc :: pcl, i)) instrs) :: aux q
  and aux = function
    | [] ->
      []
    | (pc, []) :: q ->
      aux0 pc [pc] q
    | (pc, instrs) :: q ->
      (pc, List.map (fun i -> ([pc], i)) instrs) :: aux q
  in
  aux ir
...
(* Code flattening *)
let rec last = function [] -> assert false | [(x, _)] -> x | _ :: q -> last q
...
let print_unflattened_code code =
  List.iter
    (fun (i, instrs) ->
       List.iter
         (fun (pc, op) -> Printf.printf "%3d (%3d) : %s\n" i pc (print_instr op))
         instrs )
    code ;
  print_newline ()
  ...
  let instrs =
    Array.of_list
      (List.map
         (function
           | Goto pc ->
             Goto (Ptmap.find pc map)
           | Ifd (g, pc) ->
             Ifd (g, Ptmap.find pc map)
           | ins ->
             ins )
         instrs)
  (* Change to new pcs *)
         ...
         | JCode.OpInvalid ->
           ...
           let rec check_is_invalid i i_end =
             ...
             | _ ->
               assert false
           in
           check_is_invalid eh.JCode.e_start eh.JCode.e_end ;
           false
         | _ ->
           true )
      code.JCode.c_exc_tbl
...
let jsr_ir2bc_post_treatment (assoc : (int * int) list) (ir2bc : int array) :
  ...
  let rec update_pp pp pp_list =
    ...
    | [] ->
      pp
    | (inline_pp, bc_pp) :: tail ->
      if inline_pp = pp then bc_pp else update_pp pp tail
  in
...
module Live = struct
  module Env = struct
    ...
    include Set.Make (struct
        ...
        let compare v1 v2 = index v1 - index v2
      end)
  ...
  let transfer_to_string = function
    ...
    | f :: q ->
      fun_to_string f
      ^ List.fold_right (fun f s -> ";" ^ fun_to_string f ^ s) q ""

  let eval_transfer = function
    | GenVars l ->
      fun ab -> List.fold_right (fun e -> Env.union (Env.vars e)) l ab
    | Kill x ->
      fun ab -> Env.remove x ab
  ...
  let rec all_expr_in_formula acc = function
    ...
    | Atom (_, e1, e2) ->
      e1 :: e2 :: acc
    | And (f1, f2) | Or (f1, f2) ->
      all_expr_in_formula (all_expr_in_formula acc f1) f2
  ...
  let gen_instrs last i = function
    ...
    | InvokeStatic (Some x, _, _, le) ->
      [([GenVars le; Kill x], i + 1)]
    | InvokeVirtual (Some x, e, _, _, le)
    | InvokeNonVirtual (Some x, e, _, _, le) ->
      [([GenVars (e :: le); Kill x], i + 1)]
    | MonitorEnter e | MonitorExit e ->
      [([GenVars [e]], i + 1)]
    | AffectStaticField (_, _, e) ->
      [([GenVars [e]], i + 1)]
    | AffectField (e1, _, _, e2) ->
      [([GenVars [e1; e2]], i + 1)]
    | AffectArray (e1, e2, e3) ->
      [([GenVars [e1; e2; e3]], i + 1)]
    | InvokeDynamic (None, _, _, le) | InvokeStatic (None, _, _, le) ->
      [([GenVars le], i + 1)]
    | InvokeVirtual (None, e, _, _, le) | InvokeNonVirtual (None, e, _, _, le)
      ->
      [([GenVars (e :: le)], i + 1)]
    | MayInit _ | Nop ->
      [([], i + 1)]
  ...
  let run m =
    Iter.run
      ...
      ; Iter.init_value= (fun _ -> Env.empty)
      ; (* useless here since we iterate from bottom *)
        Iter.verbose= false
...
module SSA = struct
  ...
  let dominator instr_array preds =
    let all =
      JUtil.foldi (fun i _ -> Ptset.add i) (Ptset.singleton (-1)) instr_array
    in
    let dom = Array.init (Array.length instr_array) (fun _ -> all) in
    let get_dom i = if i < 0 then Ptset.singleton (-1) else dom.(i) in
    let rec inter_list = function
      ...
      | [x] ->
        get_dom x
      | x :: q ->
        Ptset.inter (get_dom x) (inter_list q)
    in
  ...
  let make_idom_tree aux =
    let assoc_list =
      JUtil.foldi (fun i s l -> (Ptset.choose s, i) :: l) [] aux
    in
  ...
  let show_digraph instr_array succs =
    ...
    Array.iteri
      (fun i _ ->
         List.iter (fun j -> Printf.fprintf f "  n%d -> n%d;\n" i j) (succs i)
      )
      instr_array ;
    Printf.fprintf f "}\n" ;
    close_out f
  ...
  let place_phi_nodes m n var_defs domf live =
    ...
    let place_node n v =
      place := Ptmap.add ~merge:Ptset.union n (Ptset.singleton v) !place
    in
  ...
  let debug_code m phi_nodes children var_defs search_h succs =
    Printf.printf "params(%s)\n"
      (JUtil.print_list_sep_id ","
         (List.map (fun (_, x) -> var_name_g x) m.bir_params)) ;
    Array.iteri
      (fun i op ->
    ...
    Ptmap.iter
      (fun v defs ->
         match v with
         | v when v = heap_index ->
           () (*not contained in ir_code.bir_vars *)
         | _ ->
           Printf.printf "   %s: {%s}\n"
    ...
    Printf.printf "search: %s\n"
      (JUtil.print_list_sep_id "::"
         (List.map
            (fun x ->
               Printf.sprintf "%d(%s)" x
                 (JUtil.print_list_sep_id " "
                    (List.map string_of_int (children x))) )
            search_h)) ;
  ...
  let use_bcvars =
    let rec vars acc = function
      | Const _ ->
        acc
      | Var (_, x) ->
        if catch_var x then acc else Ptset.add (index x) acc
      | Field (e, _, _) ->
        vars (Ptset.add heap_index acc) e
      | Unop (_, e) ->
        vars acc e
      | Binop (ArrayLoad _, e1, e2) ->
        vars (vars (Ptset.add heap_index acc) e1) e2
      | Binop (_, e1, e2) ->
        vars (vars acc e1) e2
      | StaticField _ ->
    ...
    | Ifd ((_, e1, e2), _) ->
      vars (vars Ptset.empty e1) e2
    | Goto _ | MayInit _ | Nop | Return None ->
      Ptset.empty
    | Throw e
    | Return (Some e)
    | AffectVar (_, e)
    ...
    | InvokeDynamic (_, _, _, le) ->
      List.fold_left vars (Ptset.add heap_index Ptset.empty) le
    | InvokeVirtual (_, e, _, _, le) | InvokeNonVirtual (_, e, _, _, le) ->
      List.fold_left vars (Ptset.add heap_index Ptset.empty) (e :: le)
    | AffectArray (e1, e2, e3) ->
      vars (vars (vars (Ptset.add heap_index Ptset.empty) e1) e2) e3
    | Check c ->
  ...
  let def_bcvar = function
    ...
    | AffectVar (v, _) ->
      if catch_var v then Ptset.empty else Ptset.singleton (index v)
    | AffectField _ | AffectStaticField _ | AffectArray _ ->
      Ptset.singleton heap_index
    | _ ->
      Ptset.empty
  ...
  let map_instr def use =
    ...
    function
    | AffectField (e1, c, f0, e2) ->
      AffectField (use e1, c, f0, use e2)
    | Ifd ((c, e1, e2), pc) ->
      Ifd ((c, use e1, use e2), pc)
    | Goto i ->
      Goto i
    | Throw e ->
      Throw (use e)
    ...
    | Return (Some e) ->
      Return (Some (use e))
    | AffectVar (x, e) ->
      AffectVar (def x, use e)
    | MonitorEnter e ->
      MonitorEnter (use e)
    | MonitorExit e ->
      MonitorExit (use e)
    | AffectStaticField (c, f0, e) ->
    ...
    | InvokeStatic (None, c, ms, le) ->
      InvokeStatic (None, c, ms, List.map use le)
    | InvokeStatic (Some x, c, ms, le) ->
      InvokeStatic (Some (def x), c, ms, List.map use le)
    | InvokeDynamic (None, bm, ms, le) ->
      InvokeDynamic (None, bm, ms, List.map use le)
    | InvokeDynamic (Some x, bm, ms, le) ->
      InvokeDynamic (Some (def x), bm, ms, List.map use le)
    | InvokeVirtual (None, e, c, ms, le) ->
      InvokeVirtual (None, use e, c, ms, List.map use le)
    | InvokeVirtual (Some x, e, c, ms, le) ->
      InvokeVirtual (Some (def x), use e, c, ms, List.map use le)
    | InvokeNonVirtual (None, e, c, ms, le) ->
      InvokeNonVirtual (None, use e, c, ms, List.map use le)
    | InvokeNonVirtual (Some x, e, c, ms, le) ->
      ...
      Check
        ( match c with
          | CheckArrayBound (e1, e2) ->
            CheckArrayBound (use e1, use e2)
          | CheckArrayStore (e1, e2) ->
            CheckArrayStore (use e1, use e2)
          | CheckNullPointer e ->
            CheckNullPointer (use e)
          | CheckNegativeArraySize e ->
            CheckNegativeArraySize (use e)
          | CheckCast (e, t) ->
            CheckCast (use e, t)
          | CheckArithmetic e ->
            CheckArithmetic (use e)
          | CheckLink op ->
            CheckLink op )
    ...
    Array.iteri
      (fun i ins ->
         ...
         | _ ->
           add_pred (i + 1) i )
      m.bir_code ;
    List.iter (fun (i, e) -> add_pred e.e_handler i) (bir_exception_edges m) ;
    ...
    Array.iteri
      (fun i ins ->
         ...
         | _ ->
           add i (i + 1) )
      m.bir_code ;
    List.iter (fun (i, e) -> add i e.e_handler) (bir_exception_edges m) ;
    let succs = Array.map Ptset.elements succs in
    let succs i = if i = -1 then [0] else succs.(i) in
    succs
  ...
  let rename m var_defs children preds succs phi_nodes =
    ...
    in
    let phi_nodes i =
      try Ptmap.find i phi_nodes with Not_found -> Ptmap.empty
    in
    let search_h = ref [] in
    let top_s i x =
      try
    ...
    let rec search x =
      ...
      let def =
        if x < 0 then
          ...
          List.fold_right
            (fun (_, x) -> Ptset.add (index x))
            m.bir_params
            (Ptset.singleton heap_index)
        else def_bcvar m.bir_code.(x)
      in
      Ptmap.iter
        (fun v _ ->
           let xmap =
             try Ptmap.find x !rename_def_phi with Not_found -> Ptmap.empty
           in
           let i = Ptmap.find v !c in
           rename_def_phi :=
             Ptmap.add x
               (Ptmap.add v i xmap) (* at point x, v |-> v_i *)
               !rename_def_phi ;
      ...
      ( if x >= 0 then
          ...
          rename_use.(x)
          <- Ptset.fold (fun v -> Ptmap.add v (top_s x v)) var_defs Ptmap.empty
      ) ;
      Ptset.iter
        (fun v ->
           let xmap =
             try Ptmap.find x !rename_def with Not_found -> Ptmap.empty
           in
           let i = Ptmap.find v !c in
           rename_def := Ptmap.add x (Ptmap.add v i xmap) !rename_def ;
           s := Ptmap.add v (i :: Ptmap.find v !s) !s ;
           c := Ptmap.add v (i + 1) !c )
        def ;
      List.iter
      ...
      Ptmap.iter (fun v _ -> pop_s v) (phi_nodes x)
    in
    search (-1) ;
    ( (fun i ->
  ...
  let tarjan_idom succ pred r n =
    ...
    let rec dfs v =
      ...
      incr nb_dfs ;
      List.iter
        (fun w -> if semi.(w) = -1 then ( parent.(w) <- v ; dfs w ))
        (succ v)
    in
    let rec compress v =
    ...
    in
    let eval v = if ancestor.(v) = -1 then v else ( compress v ; label.(v) ) in
    ...
    let idom i = try dom.(i) with Invalid_argument _ -> assert false in
    let children i =
      if i = -1 then [0]
      else
        try children.(i)
  ...
  let run ir_code live =
    ...
    *)
    ...
    let phi_nodes i =
      try Ptset.elements (Ptmap.find i phi_nodes) with Not_found -> []
    in
    (* ((fun i -> dom.(i)), *)
  ...
  let vars_to_string s =
    Printf.sprintf "{%s}" (JUtil.print_list_sep "," var_name_g s)
  ...
  let debug ir_code
      ...
      , phi_nodes
      , var_defs
      , (_rename_def, _rename_def_phi, rename_use, phi_nodes')
      , _preds ) =
    ...
    let var_defs =
      JUtil.foldi
        (fun pc _ def_map ->
           ...
           List.fold_left
             (fun def_m v_i ->
                Ptmap.add ~merge:Ptset.union v_i (Ptset.singleton pc) def_m )
             def_map var_list )
        var_defs ir_code.bir_code
    in
    Ptmap.iter
      (fun v defs ->
         match v with
         ...
         Ptmap.iter
           (fun v i ->
              ...
              | v when v = heap_index ->
                () (*not contained in ir_code.bir_vars *)
              | _ ->
                Printf.printf " %s_[%d]" (var_name_g ir_code.bir_vars.(v)) i )
           rename_use ;
         print_newline () ;
           ...
           (fun v args ->
              if v <> heap_index then
                ...
                Printf.printf "      %s := PHI(%s)\n" v
                  (JUtil.print_list_sep_id ","
                     (List.map (Printf.sprintf "%s_%d" v) (Array.to_list args)))
           )
           phi_nodes ;
         Printf.printf "%s%3d: %s\n"
  ...
  let get_heap_index def use phi_node code =
    ...
    for i = 0 to n - 1 do
      if Ptset.mem heap_index (def_bcvar code.(i)) then
        out_index := Ptmap.add i (def i) !out_index ;
      if Ptset.mem heap_index (use_bcvars code.(i)) then
        in_index := Ptmap.add i (use i) !in_index ;
      try
        match snd (phi_node i) with
        | None ->
          ()
        | Some (x, args) ->
          phi := Ptmap.add i (x, args) !phi
      with Not_found -> ()
    done ;
  ...
  let transform_from_ir ir_code =
    ...
    let debug i msg =
      ...
      debug ir_code run
    in
    let _, _, _, _, (rename_def, rename_def_phi, rename_use, phi_nodes'), preds
      =
      run
    in
    ...
    let use i =
      ...
      | x -> (
          if catch_var x then make_var x 0
            ...
            with Not_found ->
              debug i (Printf.sprintf "use var %s lookup" (var_name_g x)) ;
              assert false )
    in
    let phi_nodes i =
      try
        Ptmap.fold
          (fun v args (l, o) ->
             if v = heap_index then (l, Some (rename_def_phi i v, args))
               ...
               ({def= x; use= Array.map (make_var x_ir) args} :: l, o) )
          (phi_nodes' i) ([], None)
      with Not_found ->
        debug i "phi lookup" ;
        assert false
    in
    let code =
      Array.mapi (fun i -> map_instr (def i) (use i)) ir_code.bir_code
    in
    let mem_in, mem_out, mem_phi =
      get_heap_index
        (fun i -> rename_def i heap_index)
        (fun i -> Ptmap.find heap_index (rename_use i))
        phi_nodes ir_code.bir_code
    in
    let exc_t =
      List.map (map_exception_handler make_var) ir_code.bir_exc_tbl
    in
    let params =
      List.map (fun (t, x) -> (t, make_var x 0)) ir_code.bir_params
    in
    let preds =
    ...
    in
    let vars =
      if Array.length ir_code.bir_vars = 0 then [||] else make_array_var dico
    in
    { bir_vars= vars
    ; bir_dico= dico
    ; bir_params= params
...
let phi_nodes get ioc ms =
  ...
  | ConcreteMethod cm -> (
      ...
      | Java laz ->
        ...
        if Array.length code.bir_phi_nodes <> 0 then fun pp ->
          (code.bir_phi_nodes.(pp), code.bir_preds.(pp))
        else fun _ -> ([], Array.make 0 0)
      | Native ->
        fun _ -> ([], Array.make 0 0) )
  | AbstractMethod _ ->
    fun _ -> ([], Array.make 0 0)
...
let inst_html program ioc ms pp op =
  ...
  | AffectStaticField (ccs, fs, e) ->
    ...
    let p2 = simple_elem (Printf.sprintf ":= %s" (print_expr e)) in
    [p1; p2]
  | AffectField (e1, ccs, fs, e2) ->
    let p1 = field_elem program ~called_cname:(print_expr e1) cs ccs fs in
    let p2 = simple_elem (Printf.sprintf ":= %s" (print_expr e2)) in
    [p1; p2]
  | New (x, ccs, _, le) ->
    let v = TObject (TClass ccs) in
    let p1 = simple_elem (Printf.sprintf "%s := new" (var_name_g x)) in
    let p2 = value_elem program cs v in
    let p3 =
      simple_elem
        (Printf.sprintf "(%s)" (JUtil.print_list_sep ", " print_expr le))
    in
    [p1; p2; p3]
  | NewArray (x, v, le) ->
    let p1 = simple_elem (Printf.sprintf "%s := new" (var_name_g x)) in
    ...
    let p3 =
      simple_elem
        (Printf.sprintf "%s"
           (JUtil.print_list_sep ""
              (fun e -> Printf.sprintf "[%s]" (print_expr e))
              le))
    in
    [p1; p2; p3]
  ...
  | InvokeStatic (Some x, ccs, cms, le) ->
    let p1 = simple_elem (Printf.sprintf "%s :=" (var_name_g x)) in
  ...
  | InvokeVirtual (r, e1, k, cms, le) -> (
      let p2 =
        ...
        | VirtualCall o ->
          let ccs =
            match o with TClass ccs -> ccs | _ -> JBasics.java_lang_object
          in
          invoke_elem ~called_cname:(print_expr e1) program cs ms pp ccs cms
        | InterfaceCall ccs ->
          invoke_elem ~called_cname:(print_expr e1) program cs ms pp ccs cms
      in
      let p3 =
      ...
      simple_elem
        ( match r with
          | None ->
            print_expr e1 ^ "."
          | Some x ->
            Printf.sprintf "%s := %s." (var_name_g x) (print_expr e1) )
    in
    let p2 = invoke_elem program cs ms pp ccs cms in
    let p3 =
      simple_elem
  ...
  | MayInit ccs ->
    ...
    [p1; p2]
  | Check (CheckCast (e, t)) ->
    let p1 = simple_elem (Printf.sprintf "checkcast %s:" (print_expr e)) in
    let p2 = value_elem program cs (TObject t) in
    [p1; p2]
  | _ ->
    [simple_elem (print_instr op)]
...
let inst_html get program ioc ms pp op =
  ...
  if phi_nodes = [] then res
    ...
    simple_elem
      (Printf.sprintf "(preds(%d) := (%s)): " pp
         (JUtil.print_list_sep ", " string_of_int (Array.to_list preds)))
    :: List.fold_right
      (fun phi_n elts ->
         let el =
...
let iter_exc_handler f code = List.iter f code.bir_exc_tbl
  ...
  ( match cm with
    | {Javalib.cm_implementation=Java code; _}
      ...
      Array.iteri
        (fun pp op ->
           match op with
           | InvokeStatic _ | InvokeVirtual _ | InvokeNonVirtual _ ->
    ...
    | ConcreteMethod cm ->
      ...
      Ptmap.iter
        (fun _pp cmsSet ->
           ClassMethodSet.iter
             (fun cms ->
                if ClassMethodSet.mem cms !history then ()
                else (
                  history := ClassMethodSet.add cms !history ;
                  workset := cms :: !workset ) )
             cmsSet )
        mcalls ;
      calls := methodcalls2callsite cur_cn cm.cm_signature mcalls @ !calls
    | AbstractMethod _ ->
      ()
  done ;
  !calls
...
let bir_code_map (f : instr -> instr) (m : bir) : bir =
  { m with
    bir_code= Array.init (Array.length m.bir_code) (fun i -> f m.bir_code.(i))
  }
...
let bir_resolve_field_in_expr prog (e : expr) : expr =
  let rec aux e =
    match e with
    | Field (x, cls, fs) ->
      Field (aux x, bir_resolve_field prog cls fs, fs)
    | StaticField (cls, fs) ->
      StaticField (bir_resolve_field prog cls fs, fs)
    | Unop (op, e') ->
      Unop (op, aux e')
    | Binop (op, e1, e2) ->
...
let bir_field_resolve_in_code prog (inst : instr) : instr =
  ...
  let resolve_check = function
    | CheckNullPointer e ->
      CheckNullPointer (resolve e)
    | CheckArrayBound (a, idx) ->
      CheckArrayBound (resolve a, resolve idx)
    | CheckArrayStore (a, e) ->
      CheckArrayStore (resolve a, resolve e)
    | CheckNegativeArraySize e ->
      CheckNegativeArraySize (resolve e)
    | CheckCast (e, t) ->
      CheckCast (resolve e, t)
    | CheckArithmetic e ->
      CheckArithmetic (resolve e)
    | CheckLink op ->
      CheckLink op
  in
  match inst with
  ...
  | AffectArray (a, idx, e) ->
    AffectArray (resolve a, resolve idx, resolve e)
  | AffectField (e, c, fs, e') ->
    AffectField (resolve e, bir_resolve_field prog c fs, fs, resolve e')
  | AffectStaticField (c, fs, e) ->
    AffectStaticField (bir_resolve_field prog c fs, fs, resolve e)
  | Ifd ((op, e1, e2), pc) ->
    Ifd ((op, resolve e1, resolve e2), pc)
  | Throw e ->
    Throw (resolve e)
  | Return (Some e) ->
    Return (Some (resolve e))
  | New (x, c, tl, args) ->
    New (x, c, tl, List.map resolve args)
  | NewArray (x, t, el) ->
    NewArray (x, t, List.map resolve el)
  | InvokeStatic (x, c, ms, args) ->
    InvokeStatic (x, c, ms, List.map resolve args)
  | InvokeDynamic (x, bm, ms, args) ->
    InvokeDynamic (x, bm, ms, List.map resolve args)
  | InvokeVirtual (x, e, k, ms, args) ->
    InvokeVirtual (x, resolve e, k, ms, List.map resolve args)
  | InvokeNonVirtual (x, e, c, ms, args) ->
    InvokeNonVirtual (x, resolve e, c, ms, List.map resolve args)
  | MonitorEnter e ->
    MonitorEnter (resolve e)
  | MonitorExit e ->
...
module MakeCodeExcFunctions = struct
  open AdaptedASTGrammar

  let to_plugin_warning' get_code get_exc jm pp_warn_map ast_of_i ast_of_e =
    let handlers_pc_map code =
      if Ptmap.is_empty pp_warn_map then Ptmap.empty
      else
        List.fold_left
          (fun map eh -> Ptmap.add eh.e_handler eh.e_catch_type map)
          Ptmap.empty (get_exc code)
    in
        ...
        try
          Some
            ( match Ptmap.find pc handlers_pc_map with
              | None ->
                PreciseLineWarning (msg, Statement Finally)
              | Some cn ->
                PreciseLineWarning (msg, Statement (Catch cn)) )
        with Not_found -> None
      in
  ...
  let inst_disp' printf pp cod =
    let printf_esc i =
      JPrintUtil.replace_forb_xml_ch ~repl_amp:true (printf i)
    in
    printf_esc cod.bir_code.(pp)

  let get_source_line_number pp code = bir_get_source_line_number pp code
end
...
let simplify_CheckNullPointer (cm : 'a concrete_method) (b : bir) : unit =
  if not cm.cm_static then
    ...
    if
      ExtArray.Array.for_all
        (* we check that no instruction assigns the [this] variable *)
        (fun ins ->
           match ins with
           | AffectVar (x, _)
           | NewArray (x, _, _)
...
module MakeBirLikeFunctions (* (S : JBir.Internal.CodeInstrSig) = *) = struct
  include IRUtil
  ...
  let find_ast_node_of_expr = function
    | Const _ ->
      None
    | Binop (ArrayLoad vt, _, _) ->
      Some
        (AdaptedASTGrammar.Expression
           (AdaptedASTGrammar.ArrayAccess (Some vt)))
    | Binop (_, _, _) ->
      None
    ...
    | Var (vt, var) ->
      Some
        (AdaptedASTGrammar.Name
           (AdaptedASTGrammar.SimpleName (var_name_g var, Some vt)))
    | Field (_, _, fs) | StaticField (_, fs) ->
      Some
        (AdaptedASTGrammar.Name
           (AdaptedASTGrammar.SimpleName (fs_name fs, Some (fs_type fs))))

  let find_ast_node = function
    | Goto _ | MayInit _ | Nop ->
      None
    | AffectField (_, _, fs, _) | AffectStaticField (_, fs, _) ->
      Some
        (AdaptedASTGrammar.Expression
           (AdaptedASTGrammar.Assignment
              (AdaptedASTGrammar.SimpleName (fs_name fs, Some (fs_type fs)))))
    | Ifd ((_cmp, _e1, _e2), _pc) ->
      Some (AdaptedASTGrammar.Statement AdaptedASTGrammar.If)
    | Return _ ->
    ...
    | AffectVar (v, e) ->
      Some
        (AdaptedASTGrammar.Expression
           (AdaptedASTGrammar.Assignment
              (AdaptedASTGrammar.SimpleName
                 (var_name_g v, Some (type_of_expr e)))))
    | MonitorEnter _e ->
      Some
        (AdaptedASTGrammar.Statement (AdaptedASTGrammar.Synchronized true))
    | MonitorExit _e ->
      Some
        (AdaptedASTGrammar.Statement (AdaptedASTGrammar.Synchronized false))
    | NewArray (_v, vt, _le) ->
      Some
    ...
    | InvokeStatic (_, cn, ms, _le) | InvokeNonVirtual (_, _, cn, ms, _le) ->
      Some
        (AdaptedASTGrammar.Expression
           (AdaptedASTGrammar.MethodInvocationNonVirtual (cn, ms)))
    | InvokeDynamic _ ->
