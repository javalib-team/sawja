...
(** [check] is the type of JBir assertions. They are generated by the
    transformation so that execution errors arise in the same order in
    the initial bytecode program and its JBir version. Next to each of
    them is the informal semantics they should be given.*)
...
(** [transform ~bcv ~ch_link ~formula ~formula_cmd cm jcode] transforms the 
    ...
    - [?folding]: By default Sawja want to fold allocations and constructors 
