...
let get_code pp = 
  ...
  let impl = meth.cm_implementation in
    match impl with 
      ...
      | Java laz -> JBirSSA.code (Lazy.force laz)
...
let static_lookup program pp =
  match get_opcode pp with
    | JBirSSA.InvokeVirtual (_,_,kind, ms,_) ->
        (match kind with
             JBirSSA.VirtualCall obj -> 
      	 Some (static_lookup_virtual program obj ms)
           | JBirSSA.InterfaceCall cs -> 
...
let normal_successors pp =
  ...
  let exceptional_successors pp =
    List.map 
      (fun e -> goto_absolute pp e.JBirSSA.e_handler) (handlers pp)
