...
let compute_jump_target code =
  ...
  let rec add_interval i j set =
    if i>=j then set
      ...
      else add_interval (i+1) j set in    
  let add_handler e =
    jump_target.(e.e_handler) <- add_interval e.e_start e.e_end jump_target.(e.e_handler) in
...
(* return the set of prog points reachable from [i] and the set of OpRet at exit points *)
...
(* try to build an interval from the subroutine prog points *)
...
let inline code instrs subroutines =
  ...
  let new_code = Array.make new_size OpInvalid in
    ...
    List.iter
      (fun (i,start) ->
	 let (code_start,codes,code_exit,size,handlers_mine,handlers_global) = subroutines start in
	   ...
	   new_code.(i) <- OpGoto ((next_after_jsr code code_start)+ !current-code_start-i);
	   new_code.(!current+code_exit-code_start) <- 
	     OpGoto ((next_after_jsr code i)-(!current+code_exit-code_start));
	   handlers := 
	     (List.map (fun e -> 
			  {
      ...
      let next = next_pc new_code in
    {
      ...
      c_code = new_code;
      c_exc_tbl = 
	(List.filter (fun e -> new_code.(e.e_handler)<>OpInvalid) code.c_exc_tbl)
	  @(List.map
	      (fun e -> 
		 {
		   e_start = next e.e_start;
		   e_end = next e.e_end;
		   e_handler = e.e_handler;
		   e_catch_type = e.e_catch_type
		 }) !handlers);
...
let scan_subroutine code jump_target start =
  ...
  let (start,endd) = make_intervals_union code succs in
    ...
    let handlers = (* all handlers that catch the subroutine *)
      List.filter
	(fun e ->  
	   Ptset.exists
