...
val get_class : t -> A3Bir.t node

val get_meth : t -> A3Bir.t concrete_method

val get_pc : t -> int

val get_pp : A3Bir.t node -> A3Bir.t concrete_method -> int -> t
...
val get_first_pp : A3Bir.t program -> class_name -> method_signature -> t

(** [get_first_pp_wp n ms] gets a pointer to the first instruction
    of the method [ms] of the node [n].

    @raise NoCode if the method [ms] has no associated code.*)
val get_first_pp_wp : A3Bir.t node -> method_signature -> t

(** get internal representation. *)
val get_ir : t -> A3Bir.t

val get_opcode : t -> A3Bir.instr
...
val static_pp_lookup : A3Bir.t program -> t -> t list

(** [static_lookup program pp] returns the highest methods in the
  hierarchy that may be called from program point [pp]. All methods that may be
  called at execution time are known to implement or extend one of the class
  that this function returns. *)
val static_lookup : A3Bir.t program -> t -> A3Bir.t node list option

(** [handlers pp] returns the list of exception handlers which can be raised at
  program point [pp]. This is a surapproximation.*)
val handlers : t -> A3Bir.exception_handler list
