...
(** [check] is the type of JBirSSA assertions. They are generated by the
    transformation so that execution errors arise in the same order in the initial
    bytecode program and its JBirSSA version. Next to each of them is the informal
...
(** JBirSSA instructions are register-based and unstructured. Next to them is the
