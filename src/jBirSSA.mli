...
type phi_node = {
  ...
  use_set : VarSet.t;
  (** (simplified view of [use] without information on predecessor) Set
      of used variables in the phi node*)
}
...
type t = {
  ...
  (** map from bytecode code line to ir code line (very sparse). It
      could raise Not_found if pc is not an original bytecode pc or if
      the corresponding bytecode instruction has no predecessors and
      has been removed because it is unreachable.*)
  pc_ir2bc : int array; 
